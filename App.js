...
const Checkout = ({ flowersInCart, onProceed }) => {
  const totalCost = flowersInCart.reduce((total, flower) => total + (flower.price || 0), 0);
...
const HomeScreen = ({ navigation, userName, flowers, onAddToCart, flowersInCart, onRemoveFromCart,removeFlowerFromCart,onProceed,handleCheck  }) => {
...
const buttons = [
  { text: 'Insta', url: 'https://www.britannica.com/plant/lily' },
 
  { text: 'Telegram', url: 'https://example.com/pionees' },
...
const App = () => {
  ...
  setShowProceedButton(flowersInCart.length > 0);
};
